...
<article
	class="flex border-2 border-bauhaus-fg shadow-[4px_4px_0px_0px_#121212] transition-all duration-200 ease-out hover:-translate-y-0.5 md:border-4"
>
	<!-- Status accent bar -->
	...
	<div class="relative flex flex-1 flex-col gap-1 bg-white p-3 md:p-4">
		...
		{#if pill}
			<span
				class="absolute right-3 top-3 rounded-full px-2 py-0.5 text-xs font-bold uppercase tracking-wider {pill.bg}"
			>
				{pill.label}
			</span>
		{/if}
		...
		<div class="flex flex-wrap items-center gap-x-1 gap-y-0.5 pr-20">
			{#each show.artists as artist, i (artist.name)}
				{#if artist.match_type === 'stretch'}
					<span class="text-bauhaus-yellow text-xs">•</span>
				{/if}
		...
		<!-- Venue + recommended indicator -->
		<div class="flex items-center gap-2">
			<span class="text-xs font-medium text-bauhaus-fg/60 md:text-sm">{show.venue.name}</span>
			{#if show.recommended}
				<div
					class="h-2 w-2 flex-shrink-0 rounded-full bg-bauhaus-fg"
					title="Recommended"
				></div>
			{/if}
		</div>
